...
export const generateExercises = async (options: ExerciseOptions, modelName: string = DEFAULT_AI_MODEL): Promise<Exercise[]> => {
  ...
  // Détection du type QCM
  const isQCMType = type.includes("QCM") || type.includes("Question à choix multiple");
  ...
  if (isQCMType) {
      ...
      if (selectedChapters && selectedChapters.length > 0) {
          chapterInstruction = `SYNTHÈSE MULTI-CHAPITRES : ${selectedChapters.map(c => c.chapter).join(' + ')}.`;
      }

      const isArabicQCM = isArabicSubject;

      if (isArabicQCM) {
          prompt = `
            تقمص دور مفتش تربوي بوزارة التربية الوطنية والتعليم الأولي والرياضة (المغرب).
            المهمة: إنشاء أسئلة متعددة الخيارات (QCM) للمستوى: ${level}.
            المادة: ${subject}.
            ${chapterInstruction}
            مستوى الصعوبة: ${ARABIC_MAP[difficulty] || difficulty}.
            عدد الأسئلة: ${options.exerciseCount || 3}.

            **تعليمات صارمة للشكل والمحتوى:**
            1. **هيكلية السؤال**:
               - صياغة سؤال واضح ومباشر.
               - توفير 4 خيارات (أ، ب، ج، د) مع خيار صحيح واحد فقط.
            2. **المحتوى**:
               - يجب أن تغطي الأسئلة المفاهيم الأساسية للفصل.
               - تجنب الغموض في الخيارات.
            3. **التصحيح**:
               - توفير شرح موجز للإجابة الصحيحة.
            ...
            **تنسيق الإخراج (JSON):**
            [{
               ...
               "corrige": "شرح الإجابة الصحيحة..."
            }]
          `;
      } else {
          prompt = `
            Rôle : Inspecteur Pédagogique (Ministère de l'Éducation Nationale - Maroc).
            Tâche : Créer des questions à choix multiples (QCM) pour le niveau : ${level}.
            Matière : ${subject}.
            ${chapterInstruction}
            Difficulté : ${difficulty}.
            Nombre de questions : ${options.exerciseCount || 3}.

            **INSTRUCTIONS DE FORME (STRICTES) :**
            1. **Structure de la question**:
               - Formuler une question claire et précise.
               - Fournir 4 choix de réponse (A, B, C, D) avec une seule bonne réponse.
            2. **Contenu**:
               - Les questions doivent couvrir les concepts clés du chapitre.
               - Éviter les réponses ambiguës ou évidentes.
            3. **Correction**:
               - Fournir une brève explication de la bonne réponse.
            ...
            [{
               ...
               "correctAnswerIndex": 0,
               "corrige": "Explication de la réponse correcte..."
            }]
          `;
      }
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    items: {
                        ...
                        required: ["enonce", "choices", "correctAnswerIndex", "corrige"]
                    }
                }
            }
      });
      ...
      const result = parsed.map((ex: any, i: number) => ({
          ...
          title: ex.title || (isArabicSubject ? `سؤال ${i+1}` : `Question ${i+1}`)
      }));
  ...
  // Code original pour les exercices non-QCM
  if (isArabicSubject) {
